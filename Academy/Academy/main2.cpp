...
public:
	...
	virtual std::ofstream& write(std::ofstream& ofs)const
	{
		...
		ofs.width(TYPE_WIDTH); ofs << write << std::string(strchr(typeid (*this).name(), ' ') + 1) ; //+":" //Название класса
		ofs.width(LAST_NAME_WIDTH); ofs << write << last_name + "| ";
		ofs.width(FIRST_NAME_WIDTH); ofs << write << first_name + "| ";
		ofs.width(AGE_WIDTH); ofs << write << age << "| ";
		return ofs;
	}
...
public:
	...
	std::ofstream& write(std::ofstream& ofs)const override //переопределяем
	{
		...
		ofs.width(SPECIALITY_WIDTH); ofs << write << speciality << "| ";
		ofs.width(GROUP_WIDTH); ofs << write << group << "| ";
		ofs.width(RATING_WIDTH); ofs << write << rating << "| ";
		ofs.width(ATTENDANCE_WIDTH); ofs << write << attendance << "| ";
...
public:
	...
	std::ofstream& write(std::ofstream& ofs)const override //переопределяем
	{
		...
		ofs.width(SPECIALITY_WIDTH); ofs << write << speciality << "| ";
		ofs.width(GROUP_WIDTH); ofs << write << experience << "| ";
...
public:
	...
	std::ofstream& write(std::ofstream& ofs)const override //переопределяем
	{
		...
		ofs.width(DIPLOM_NAME_WIDTH); ofs << write << diplom_name << "| ";
		ofs.width(BALL_WIDTH); ofs << write << ball << "| ";
		return ofs;
	}
	std::ifstream& read_l(std::ifstream& ifs) override
	{
...
Human** Read(const std::string& filename, int& n)//
{
	...
	else
	{
		...
		group = new Human * [temp_size] {};//создаем только указатели, не сами объекты
		...
		for (int i = 0; i < temp_size; i++)
		{
			...
			if (temp_class == "Human")
			{
				...
				Temp1 = new Human(HUMAN_GIVE_PARAMETRS);
				Temp1->info();
				//group1[i] = Temp1;
				//cout << group1[i];
			}
